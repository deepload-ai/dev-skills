...
name: refactor2docs
description: |
  Use when analyzing large codebases to generate comprehensive documentation for refactoring projects.
  Triggers on: code analysis requests, documentation generation, architecture documentation,
  module design, refactoring guidance.
  DO NOT trigger: conceptual questions about refactoring, general programming questions,
  runtime debugging.
---
...
## 0. 行为准则（整个会话有效，不因对话长度放松）

❗ **每个发现必须引用来源** — 每次输出前自检
❗ **禁止输出膨胀** — 简洁回答，禁止"让我来分析..."、"首先我们需要..."等开场白
❗ **禁止重复已知信息** — 用户刚说的路径、文件刚读的内容不重复
...
**Checkpoint 输出**: `✅ Step 1 完成: 发现 {X} 个模块, {Y} 个文件, 依赖关系图已生成`
**完成性要求**: 已识别模块数 == 预期模块数
...
**步骤**:
1. **架构分析** — 分层架构识别、数据流分析、依赖关系梳理
2. **模块分析** — 核心类/接口分析、关键算法分析、设计模式识别
3. **安全分析** — 权限机制、数据流安全、潜在风险点

**Checkpoint 输出**: `✅ Step 2 完成: {X} 个模块已分析, 架构图/类图/流程图/风险清单已产出`
**完成性要求**: 已分析模块数 == Step 1 识别的模块数

### Step 3: 文档生成阶段

**目标**: 生成完整的文档集

**Checkpoint 输出**: `✅ Step 3 完成: 已生成 {X} 个文档, 覆盖 8 层架构`
**完成性要求**: 已生成文档数 == 8 层架构要求的文档数
...
**Checkpoint 输出**: `✅ Step 4 完成: 完整性检查 {X}/{Y} 通过, Mermaid图表 {Z} 个可渲染`
**完成性要求**: 通过项数 == 检查清单总数

**失败路径**:
- 文件统计失败 → 使用 Glob 遍历 + wc 统计
- 模块识别失败 → 降级为手动分析
- 文档生成失败 → 输出当前进度 + 缺失项列表
...
- 开场白（"让我来分析..." / "首先我们需要..."）
- 工具调用描述（"我将使用 Read..."）
- 已知信息复述（用户刚说的路径、文件刚读的内容）
- 冗余解释（每点只说一次）

**长度限制**:
- 单次输出不超过 500 字
- 超过则分多次输出
- 每个 Checkpoint 后暂停等待确认

---

## 6. 依赖链声明

- **Step 1 输出** = Step 2 的输入（模块清单、文件清单）
- **Step 2 输出** = Step 3 的输入（架构图、类图、流程图）
- **禁止重新搜索** — Step 3 必须引用 Step 2 的实际产出
- **交叉验证** — 写入文件前验证数字一致性
...
**来源引用要求**: 所有结论必须引用具体来源（文件路径:行号）

**零结果处理**:
| 场景 | 正确输出 | 禁止输出 |
|------|---------|---------|
| 无模块发现 | "未在 {path} 发现模块" | 猜测模块名 |
| 无依赖关系 | "未发现显式依赖" | 编造依赖链 |
| 无文档模板 | "缺少 {type} 模板，使用通用模板替代" | 复制相似项目模板 |
...
# 查找入口文件
find src -name "index.ts" -o -name "main.ts"
...
# 验证Mermaid语法
npx mmdc -i input.md -o output.svg

# 验证meta.json
node -e "JSON.parse(require('fs').readFileSync('meta.json'))"
```
